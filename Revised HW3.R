...
print("the structure of the iris data set")
print("class, dim, colnames")
print(class(iris))
print(dim(iris))
print(colnames(iris))
...
# Output of Task A1
write.table(iris_sorted2, file="A1_sorted.txt", quote=FALSE, sep="\t", col.names = NA) 
print("Output: Save the sorted data in A1_sorted.txt")
...
write.table(iris_subset, file="A2_subset.txt", sep="\t", quote=FALSE, col.names= NA)
# Print column name of subset
colnames(iris_subset)
print("Output: Save the subset data in A2_subset.txt")
...
iris_subset2 <- read.table("A2_subset.txt",sep="\t",header=T)
print("Import the data")
iris_subset2
# Print column name of subset
print("Column name of imported data")
print(colnames(iris_subset2))
# Evaluate the structure of iris data set
print("Evaluate the structure of iris data set")
print(iris[1:4,])
print(table(iris$Species))
# Output of Task A3
write.table(iris_subset2, file="A3_column modified.txt", quote=FALSE, sep="\t", col.names = NA) 
print("Output: Save the changed subset data in A3_column modified.txt")
...
dev.off()
print("Output: Save the Scatter Plots as B1_Sepal_scatter plot.pdf")
...
# Close the PDF file and save the changes
dev.off()
print("Output: Save the modified Scatter Plots as B2_Sepal_scatter plot_limited.pdf")
...
print("The mean values for the first four columns, grouped by Species")
print(means <- aggregate(iris[,1:4], by=list(Species=iris$Species), mean))
write.table(means,file="C1_mean.txt", quote=FALSE, sep="\t", col.names = NA)
print("Output: Save the mean data as C1_mean.txt")
...
# Close the PDF file and save the changes
dev.off()
print("Output: Save the stacked bar plots as C2_Stacked bars.pdf")
...
# Close the PDF file and save the changes
dev.off()
print("Output: Save the stacked bar plots as C3_horizontal bars.pdf")
...
print("first two rows of molecular weight table")
print(my_mw[1:2,])
...
print("first two rows of targeting table")
print(my_target[1:2,])
# Merging Data Frames 
...
# Output Merged file
write.table(my_mw_target2, file="D1_merge.txt", quote=FALSE, sep="\t", col.names = NA)
write.table(my_mw_target2z, file="D2_join.txt", quote=FALSE, sep="\t", col.names = NA)
print("Output: Save the merged data as D1_merge.txt")
print("Output: Save the joined data as D2_join.txt")
...
write.table(my_mw_target2a, file="E_repalce.txt", quote=FALSE, sep="\t", col.names = NA)
print("Output: Save the replaced data as E_repalce.txt")
...
# Output subset file
write.table(subset_target, file="F_filter.txt", quote=FALSE, sep="\t", col.names = NA)
print("Output: Save the filtered data as F_filter.txt")
# Check the result 
print("Does the MW of tergets in the subset greater then 4,000")
print(min(subset_target$MW)> 4000)
print("Does the MW of tergets in the subset less then 5,000")
print(max(subset_target$MW)< 5000)
print("Range of the MV")
print(range(subset_target$MW))
...
# Output query data
write.table(Target1, file="G1_in.txt", quote=FALSE, sep="\t", col.names = NA)
write.table(Target2, file="G2_row index.txt", quote=FALSE, sep="\t", col.names = NA)
print("Output: Save the selectd data using %in% method as G1_in.txt")
print("Output: Save the selectd data using row index method as G2_row index.txt")
# The results of these two methods are same, both return a data.frame
identical_value = all_equal(Target1,Target2)
print("Do the two methods generate a same result?")
print(identical_value)
print("
...
print("Compare time consumption using two different methods")
...
print("Time consumption of %in% method")
print(time_result1)
print("Time consumption of row index method")
print(time_result2+time_result3)
print("Doed the %in% method consumes more time than row index method using small data")
print(time_result1 > time_result2+ time_result3)
...
print("Download the R script")
# Execute the script using source function
source("exerciseRbasics.R")
print("Execute the R script")
print("Save the files at the same folder")
